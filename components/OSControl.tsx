    ...
    <div className="p-4 space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white dark:bg-card-dark p-3 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-bold uppercase text-slate-500 dark:text-slate-400">Ativas</span>
          </div>
          <p className="text-xl font-bold">{orders.filter(o => o.status !== 'Finalizada').length}</p>
        </div>
        <div className="bg-white dark:bg-card-dark p-3 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            <span className="w-2 h-2 rounded-full bg-accent-success"></span>
            <span className="text-[9px] font-bold uppercase text-slate-500 dark:text-slate-400">Pagas</span>
          </div>
          <p className="text-xl font-bold">{orders.filter(o => o.isPaid).length}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="bg-primary text-white text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-xl flex items-center gap-2 shadow-lg shadow-primary/20 transition-transform active:scale-95"
        >
          <span className="material-symbols-outlined text-xs">add</span> NOVA OS
        </button>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar py-2 border-b border-slate-100 dark:border-slate-800">
        {filters.map((f) => (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg whitespace-nowrap transition-all ${activeFilter === f.label
          ...
          >
            ...
            <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${activeFilter === f.label ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-700'
              }`}>
              {f.count}
            </span>
          </button>
      ...
      <div className="space-y-3">
        {filteredOrders.length > 0 ? filteredOrders.map((os) => (
          <div
            ...
            className="bg-white dark:bg-card-dark rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm relative overflow-hidden group active:scale-[0.98] transition-all cursor-pointer"
          >
            ...
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1">
                <div className="flex items-center flex-wrap gap-2 mb-1">
                  <span className="text-[10px] font-black text-primary tracking-tighter">{os.id}</span>
                  <span className="text-slate-300">•</span>
                  <span className="text-[10px] font-black italic">{os.plate}</span>
                ...
                <h4 className="font-bold text-xs leading-tight text-slate-800 dark:text-slate-100">{os.task}</h4>
                <p className="text-[8px] font-bold text-slate-500 mt-1 uppercase tracking-widest">CC: {os.costCenter}</p>
              </div>
              <span className={`text-[8px] font-black px-2 py-1 rounded-md uppercase tracking-wider whitespace-nowrap ${getStatusColor(os.status)}`}>
                {os.status}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/50 pt-3 mt-1">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[9px] text-primary font-black uppercase tracking-widest"
                >
      ...
      {selectedOS && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
          <div
            className="w-full max-w-md bg-white dark:bg-card-dark rounded-t-[2rem] p-6 space-y-4 animate-in slide-in-from-bottom-full duration-500 shadow-2xl h-[90vh] overflow-y-auto no-scrollbar"
          ...
          >
              ...
              <div className="flex-1">
                ...
                {isEditing ? (
                  <input
                    ...
                    className="text-2xl font-black italic tracking-tighter uppercase w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-1 outline-none focus:ring-2 focus:ring-primary"
                  />
                ) : (
                  <h2 className="text-2xl font-black italic tracking-tighter uppercase">{selectedOS.plate}</h2>
                )}
              </div>
              <button onClick={() => { setSelectedOSId(null); setIsEditing(false); }} className="size-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center active:scale-90 transition-transform">
            ...
            <section className="space-y-3">
              ...
              <div className="bg-slate-50 dark:bg-background-dark p-3 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-3">
                <div className="space-y-1">
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-xs font-bold italic">"{selectedOS.task}"</p>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-[10px] font-bold uppercase">{selectedOS.mechanic}</p>
                    )}
                  </div>
                  <div className="space-y-1">
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-tighter">{selectedOS.costCenter}</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {!selectedOS.quoteUrl ? (
                    ...
                    <div className="relative group w-full h-24 rounded-2xl overflow-hidden border border-blue-500/30 bg-blue-50 dark:bg-blue-900/10 shadow-inner">
                      ...
                      <button onClick={() => handleRemoveDocument(selectedOS.id, 'quote')} className="absolute top-1 right-1 size-5 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg active:scale-90"><span className="material-symbols-outlined text-[10px]">close</span></button>
                    </div>
                  )}
                </div>
                <div className="space-y-1.5">
                  ...
                  {!selectedOS.invoiceUrl ? (
                    ...
                    <div className="relative group w-full h-24 rounded-2xl overflow-hidden border border-emerald-500/30 bg-emerald-50 dark:bg-emerald-900/10 shadow-inner">
                      ...
                      <button onClick={() => handleRemoveDocument(selectedOS.id, 'invoice')} className="absolute top-1 right-1 size-5 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg active:scale-90"><span className="material-symbols-outlined text-[10px]">close</span></button>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-card-dark rounded-3xl p-6 w-full max-w-xs text-center space-y-4 shadow-2xl">
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="w-full h-12 bg-accent-error text-white font-black rounded-xl uppercase tracking-widest shadow-lg shadow-accent-error/20 text-xs"
              >
                Sim, Excluir
              </button>
              <button
                onClick={() => setShowDeleteConfirm(false)}
                className="w-full h-10 text-slate-400 font-bold uppercase text-[9px]"
              >
                Voltar
              </button>
            </div>
